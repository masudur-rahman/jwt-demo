...
func ExampleParseSigned(raw string) {
	// eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOlsiTm9uZSIsIkV2ZXJ5b25lIl0sImN1c3RvbSI6IlRoZXJlIGlzIG5vIGN1c3RvbSB2YWx1ZSBhdmFpbGFibGUiLCJleHAiOjE1NTY0NTMzODAsImlzcyI6Ik1hc3VkdXIgUmFobWFuIiwibmJmIjoxNDUxNjA2NDAwLCJzdWIiOiJUZXN0aW5nIn0.DBQoFkbHshjCig_534GVZ3M450ZclvbDW-RUm8shUOg
...
func ExampleParseSignedAndEncrypted(data string) {
	...
	tok, err := jwt.ParseSignedAndEncrypted(data)
	if err != nil {
		panic(err)
	}
...
func ExampleClaims_Validate() {
	cl := jwt.Claims{
		...
		Expiry:    jwt.NewNumericDate(time.Date(2020, 1, 1, 0, 15, 0, 0, time.UTC)),
		Audience:  jwt.Audience{"leela", "fry"},
	}

	err := cl.Validate(jwt.Expected{
		Issuer: "issuer",
		Time:   time.Date(2018, 1, 1, 0, 10, 0, 0, time.UTC),
	})
	if err != nil {
		panic(err)
	}
...
func ExampleSigned_privateClaims() {
	...
	cl := jwt.Claims{
		...
		NotBefore: jwt.NewNumericDate(time.Date(2016, 1, 1, 0, 0, 0, 0, time.UTC)),
		Expiry:    jwt.NewNumericDate(time.Now().AddDate(0, 0, 5)),
		Audience:  jwt.Audience{"None", "Everyone"},
	}

	privateCl := struct {
		CustomClaim string `json:"custom"`
	}{
		"There is no custom value available",
	}

	raw, err := jwt.Signed(sig).Claims(cl).Claims(privateCl).Claims(privateCl).Claims(cl).CompactSerialize()
	if err != nil {
		panic(err)
	}
...
func ExampleSignedAndEncrypted() {
	...
	sign, err := jose.NewSigner(jose.SigningKey{jose.RS256, rsaPrivKey}, (&jose.SignerOptions{}).WithType("JWT"))

	cl := jwt.Claims{
		Subject: "nothing",
		Issuer:  "Masudur Rahman",
	}
	raw, err := jwt.SignedAndEncrypted(sign, enc).Claims(cl).CompactSerialize()
	if err != nil {
		panic(err)
	}
